...
├── cloudflare/          Cloudflare Worker: serves the React app, proxies /api (wrangler.jsonc, src/, tests)
...
|---|---|---|
| Front door | **Cloudflare Worker** ([`cloudflare/`](cloudflare)): React app on Cloudflare's edge, `/api` proxied to the API | ALB (+ CloudFront) |
...
| Storage | Cloudflare R2 / Supabase Storage | S3 private bucket + IAM task role |
...
| CI | 7 jobs | Python 3.11 and 3.13, PostgreSQL, frontend build, smoke, Docker full stack, Cloudflare Worker |
